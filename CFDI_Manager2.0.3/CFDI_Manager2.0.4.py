...
class CFDIDownloaderGUI:
    ...
    def start_download(self):
        ...
        self.log_process(f"⚙️ Configuración: {tipo_descarga} - {estado_seleccionado} ({tipo_solicitud})")
    ...
    def download_process(self):
        ...
        try:
            ...
            VERIFICAR_TIMEOUT = 90    # antes 60; margen mayor para respuestas lentas del SAT
            ...
            while True:
                ...
                try:
                    ...
                    if hasattr(verificacion_obj, 'timeout'):
                        verificacion_obj.timeout = VERIFICAR_TIMEOUT
                    ...
                    if 'timed out' in error_msg.lower() or 'timeout' in error_msg.lower():
                        ...
                        if attempt_count < MAX_TIMEOUT_RETRIES:
                            self.log_process(f"Esperando {wait_time}s antes de reintentar... ({attempt_count}/{MAX_TIMEOUT_RETRIES})")
                            for i in range(wait_time):
                                if self.cancel_requested:
                                    break
                                time.sleep(1)
                            ...
                            self.log_process(
                                ...
                                "El SAT solo no respondió a tiempo al VERIFICAR el estado. "
                                "Puedes consultarla más tarde sin volver a solicitarla."
                            )
                            messagebox.showerror("Error de Conexión", 
                                f"No se pudo VERIFICAR el estado después de {MAX_TIMEOUT_RETRIES} intentos.\n"
                                f"La solicitud sí se creó (id_solicitud: {solicitud['id_solicitud']}).\n"
                                "Guarda ese ID y vuelve a consultar más tarde; no necesitas volver a pedirla.")
                            break
                    else:
            ...
            if self.cancel_requested:
                pass
            elif resultado_exitoso:
                self.progress_bar["value"] = 100
                self.log_process("Proceso completado exitosamente.")
                messagebox.showinfo("Éxito", "Descarga completada")
            else:
                self.log_process("El proceso terminó SIN completar la descarga. Revisa el log arriba.")
